# (truncated header imports kept same as before)
import os
import time
...
# (other DB helpers: get_user_row, create_user, create_book, create_rent unchanged)
...
def serve_layout():
    ...
    if current_user.is_authenticated:
        return html.Div([
            ...
            # ADD one-shot interval to ensure initial load
...
# -----------------------------------------------------------------------------
# Flask login routes (same as before)
# -----------------------------------------------------------------------------
LOGIN_PAGE_HTML = """
<!doctype html>
<html>
...
def auth_status():
    ...
    Returns:
      - whether Flask-Login thinks the request is authenticated
      - the incoming Cookie header (to verify curl/browser sends it)
      - current Flask session contents (helps detect signature/secret problems)
    """
    return jsonify({
...
#########################################
# --- replace the load_rents_table callback with this version ---
@app.callback(
...
# --- keep the handle_table_click callback as-is (it still writes rents-action-result) ---
@app.callback(
...
@app.callback(
    Output('rents-table', 'data'),
    duplicate-error----Output('rents-action-result', 'children'),
    Input('rents-refresh', 'data'),
    Input('refresh-rents', 'n_clicks'),
    Input('load-once', 'n_intervals'),
    prevent_initial_call=False
...
def load_rents_table(refresh_token, n_clicks, _n_intervals):
    server.logger.debug("load_rents_table called; user=%s refresh_token=%s n_clicks=%s n_intervals=%s",
                        getattr(current_user, "id", None), refresh_token, n_clicks, _n_intervals)
    if not has_request_context() or not current_user.is_authenticated:
        server.logger.debug("Not authenticated in load_rents_table")
        return [], "ログインしてください"
    ...
    try:
        ...
        server.logger.debug("get_rented_books_for_user returned %d rows", len(rows))
        if not rows:
            return [], "レンタル中の本はありません"
        return rows, ""
    except Exception as e:
        server.logger.exception("Error loading rents")
        return [], "レンタル一覧の取得に失敗しました"
...
#-------Books ->rent added ---------------------------------------
# Books -> Rent 機能追加パッチ
# Paste these functions and callbacks into your existing app.py.
# - Add get_all_books() to DB helpers.
# - In serve_layout() authenticated branch, add the "借りる" UI block (see comment).
# - Add the two callbacks at the bottom with other Dash callbacks.

# --- DB helper: get all books (add near other DB helpers) ---
def get_all_books():
    conn = get_db_conn()
    try:
        with conn.cursor() as cur:
            ...
            rows = cur.fetchall()
            # return list of dict { 'id': , 'title': , 'author': }
            result = []
            for r in rows:
                bid, title, author = r
                result.append({"id": bid, "title": title or "", "author": author or ""})
            return result
    finally:
        conn.close()

# --- UI: in serve_layout() -> if current_user.is_authenticated add this block
# Insert this small block above the DataTable (just before dash_table.DataTable(...))
# Example placement (pseudo):
#
# html.H4("本を借りる"),
# html.Div([
...
#     html.Div(id='rent-book-msg', style={'marginTop': '8px', 'color': 'green'})
# ], style={'marginBottom': '12px'}),
#
# (then the existing DataTable)

# --- Callback: load books into dropdown on page load / refresh token ---
...
def load_books_dropdown(n_intervals, refresh_token):
    # Called on initial load and when rents-refresh changes (to keep list up-to-date)
    if not has_request_context() or not current_user.is_authenticated:
        return []
    try:
        books = get_all_books()
...
# --- Callback: handle "借りる" button to create a rent and refresh list ---
@app.callback(
    Output('rent-book-msg', 'children'),
    Output('rents-refresh', 'data'),
...
def handle_rent_book(n_clicks, selected_book_id):
    ...
    try:
        # book_id stored as string in dropdown; cast to int
        book_id = int(selected_book_id)
        # create_rent returns new rent id (or may raise)
        rid = create_rent(current_user.id, book_id)
        if rid:
            # trigger a refresh by updating rents-refresh store (timestamp)
