...
# --- end patch ---
'''
# Replace the OLD conflicting callback block (the one that has
# Input('rent-book-btn','n_clicks') and Input('rents-table','active_cell'))
# with this no-op stub so it does not register duplicate Outputs.

@app.callback(
    # No Outputs: prevents duplicate-output registration while keeping symbol in source if needed
    Input('rent-book-btn', 'n_clicks'),
    prevent_initial_call=True
)
def _disabled_old_rent_handler(n_clicks):
    # Disabled: logic moved to the new confirm/modal combined callback.
    raise dash.exceptions.PreventUpdate
#--------------------------------------------------------------
@app.callback(
