    ...
    try:
        ...
        for r in rentals:
            ...
            if return_date:
                ...
                # Show Return and Cancel buttons
                items.append(html.Div([
                    html.Div([
                        html.Strong(title), " — ", html.Em(author),
...
# -----------------------------------------------------------------------------
@app.callback(
    Output('load-my-rents-once', 'n_intervals'),  # trigger reload (interval will have max_intervals=1 so re-trigger effect)
...
def handle_rent_actions(return_clicks_list, cancel_clicks_list):
    ...
    by returning current n_intervals + 1 (we don't know original value; returning 1 is fine).
    """
    triggered = dash.callback_context.triggered
    if not triggered:
        raise dash.exceptions.PreventUpdate

    # prop_id example: '{"type":"return-btn","rent_id":3}.n_clicks'
    ...
    try:
        if btn_type == 'return-btn':
            ...
            # ok True if updated, False if already returned / not found
    ...
    # To refresh the list, return 1 (Interval element has max_intervals=1 so returning 1 will cause client to see a change)
    # Alternatively we could use a dcc.Store to trigger refresh; this is a lightweight approach
    return 1
