...
def get_rented_books_for_user(user_id):
    ...
    try:
        ...
        conn.close()
#-------------------------------------------------------------
def get_books_by_search(title_q, author_q):
    """
    Partial match search for books by title and author.
    If either argument is falsy/empty, it is treated as wildcard.
    Returns same shape as get_all_books().
    """
    title_like = f"%{title_q}%" if title_q else "%"
    author_like = f"%{author_q}%" if author_q else "%"
    conn = get_db_conn()
    try:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                FROM books
                WHERE title ILIKE %s AND COALESCE(author, '') ILIKE %s
                ORDER BY title
            """, (title_like, author_like))
            rows = cur.fetchall()
            return [
                {
                    "id": r[0],
...
def serve_layout():
    ...
    # 検索行
                html.Div([
                    ...
                    html.Button('検索', id='books-search-btn', n_clicks=0, style={'marginLeft': '8px'}),
...
# --- 置換: load_books_dropdown コールバックを検索対応版に置き換え ---
# 既存の load_books_dropdown 関数を下記に置き換えてください。
...
def load_books_dropdown(n_intervals, refresh_token, search_clicks, clear_clicks, title_val, author_val):
    """
    Load books into the dropdown. Behaviors:
     - initial load (load-once) / refresh_token: load all books
     - search button clicked: perform partial search by title and/or author
     - clear button clicked: clear search and load all books
    Uses callback_context to detect which control triggered the call.
    """
    if not has_request_context() or not current_user.is_authenticated:
        return []
    ...
    try:
        # If clear button was clicked, reset search filters
        if triggered_id.startswith('books-clear-btn'):
            books = get_all_books()
        # If search button clicked and any search input provided -> filtered search
        elif triggered_id.startswith('books-search-btn'):
            # treat None as empty string
            title_q = title_val or ""
            author_q = author_val or ""
            # if both empty, return all
            if not title_q and not author_q:
                books = get_all_books()
            else:
                books = get_books_by_search(title_q, author_q)
        else:
            # default case (initial load or refresh token): show all
            books = get_all_books()
