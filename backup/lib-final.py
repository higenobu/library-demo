...
def handle_rent_and_table(n_clicks, active_cell, selected_book_id, table_data):
    """
    Combined callback:
    - If triggered by rent-book-btn -> try to create a rent for selected_book_id
    - If triggered by rents-table active_cell -> handle return/cancel actions
    ...
    try:
        if trig.startswith("rent-book-btn"):
            ...
            # check duplicate active rent
            if has_active_rent(current_user.id, book_id):
                rent_msg = "既にこの本をレンタル中です"
                return rent_msg, no_update, no_update
        ...
        # return a generic error for both UI areas
