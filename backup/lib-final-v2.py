...
def open_confirm_modal(n_clicks, selected_values):
    """Open the confirmation modal and store the selection. If nothing selected, show message."""
    if not has_request_context() or not current_user.is_authenticated:
        return None, no_update, "ログインが必要です"
    if not selected_values:
        return None, no_update, "本を選んでください"
    # normalize
    if isinstance(selected_values, (str, int)):
        ids = [selected_values]
    else:
        ids = list(selected_values)
    # store normalized ints where possible
    normalized = []
    for v in ids:
        try:
    ...
    if not normalized:
        return None, no_update, "有効な本が選択されていません"
    # show modal
    style = {'display': 'block', 'position': 'fixed', 'top': 0, 'left': 0, 'width': '100%', 'height': '100%',
             'backgroundColor': 'rgba(0,0,0,0.5)', 'zIndex': 1000}
    return normalized, style, no_update
...
def combined_confirm_or_table(confirm_clicks, active_cell, confirm_selection, table_data):
    """
    ...
    Writes rents-refresh (single writer).
    """
    if not has_request_context() or not current_user.is_authenticated:
